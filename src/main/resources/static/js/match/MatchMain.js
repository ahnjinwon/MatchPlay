...
let lastTriggerEl = null; // 마지막으로 모달 연 요소 저장
...
function setupJoinQueueButton(courtId) {
  ...
  btn.addEventListener("click", function () {
    ...
    fetch('/match/savequeue', {
      ...
      body: JSON.stringify({ courtId, teamA, teamB })
    })
    .then(res => {
...
function setupOpenJoinQueueModal(courtId) {
  ...
  const triggerBtn = document.getElementById(`openJoinQueue${courtId}`);
...
document.getElementById("btnScorePlus").addEventListener("click", async () => {
  ...
  try {
    ...
    if (end) {
      const end_response = await fetch('/match/matchend', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(courtId)
      });
      const end_result = await end_response.json();
      if (end_result === 1) {
        alert("다음 매치가 시작됩니다.");
      }
    }

    // ✅ 포커스를 먼저 밖으로 빼고 모달 닫기
